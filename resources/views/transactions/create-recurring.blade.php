...
<div class="bg-white min-h-screen">
    <div class="max-w-2xl mx-auto px-4 py-8">
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Tambah Recurring Transaction</h1>
            <p class="text-gray-500 mt-1">Pilih periode, masukkan nominal, judul, dan catatan opsional saja.</p>
        </div>

        <!-- Form Card -->
        <form action="{{ route('transactions.storeRecurring') }}" method="POST" class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
            @csrf
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-2 gap-2" id="frequency-selector">
                    <label class="frequency-option cursor-pointer rounded-2xl border-2 border-gray-200 bg-white px-4 py-3 text-center transition hover:border-emerald-400" data-value="daily">
                        <input type="radio" name="recurring_frequency" value="daily" class="sr-only" @checked(old('recurring_frequency', 'monthly') === 'daily')>
                    ...
                    <label class="frequency-option cursor-pointer rounded-2xl border-2 border-gray-200 bg-white px-4 py-3 text-center transition hover:border-emerald-400" data-value="weekly">
                        <input type="radio" name="recurring_frequency" value="weekly" class="sr-only" @checked(old('recurring_frequency', 'monthly') === 'weekly')>
                    ...
                    <label class="frequency-option cursor-pointer rounded-2xl border-2 border-gray-200 bg-white px-4 py-3 text-center transition hover:border-emerald-400" data-value="monthly">
                        <input type="radio" name="recurring_frequency" value="monthly" class="sr-only" @checked(old('recurring_frequency', 'monthly') === 'monthly')>
                    ...
                    <label class="frequency-option cursor-pointer rounded-2xl border-2 border-gray-200 bg-white px-4 py-3 text-center transition hover:border-emerald-400" data-value="yearly">
                        <input type="radio" name="recurring_frequency" value="yearly" class="sr-only" @checked(old('recurring_frequency', 'monthly') === 'yearly')>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition @error('title') border-red-500 @enderror" required placeholder="Contoh: Langganan Netflix">
                @error('title')
                    <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="amount" id="amount" value="{{ old('amount') ?? '' }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition @error('amount') border-red-500 @enderror" required inputmode="numeric" pattern="[0-9]*" placeholder="Contoh: 120000">
                @error('amount')
                    <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="start_date" id="start_date" value="{{ old('start_date', now()->format('Y-m-d')) }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition @error('start_date') border-red-500 @enderror" required>
                @error('start_date')
                    <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="3" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition @error('description') border-red-500 @enderror" placeholder="Contoh: Langganan Netflix setiap bulan">{{ old('description') }}</textarea>
                @error('description')
                    <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="flex gap-3 border-t border-gray-200 pt-6">
                ...
                <button type="submit" class="flex-1 px-4 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium">
                    Simpan Recurring Transaction
                </button>
            </div>
        </form>
    </div>
