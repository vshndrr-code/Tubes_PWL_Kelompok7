...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold">Tambah Budget Baru</h1>
            <p class="text-gray-500 mt-1">Tambahkan batas anggaran untuk kategori dan periode tertentu.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow p-6">
        <form action="{{ route('budgetings.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="limit_amount" name="limit_amount" type="number" value="{{ old('limit_amount') }}" required class="mt-1 block w-full rounded-xl border border-gray-200 bg-slate-50 px-4 py-3 shadow-sm focus:border-emerald-500 focus:ring-emerald-500/20" placeholder="0" />
                ...
                <div>
                    ...
                    <input id="year" name="year" type="number" value="{{ old('year', date('Y')) }}" required class="mt-1 block w-full rounded-xl border border-gray-200 bg-slate-50 px-4 py-3 shadow-sm focus:border-emerald-500 focus:ring-emerald-500/20" placeholder="2026" />
                    @error('year')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
            </div>

            <div class="flex justify-end pt-4 border-t border-gray-200">
                <button type="submit" class="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3 rounded-xl">Simpan Budget</button>
            </div>
        </form>
    </div>
</div>
