<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Ubah Budget</h2>
                <p class="text-sm text-gray-500">Perbarui batas pengeluaran untuk kategori yang dipilih.</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg">
                <div class="bg-gradient-to-r from-sky-500 to-sky-600 px-6 py-4">
                    <h3 class="text-xl font-semibold text-white">Ubah Budget</h3>
                    <p class="text-sky-100">Perbarui data budget sesuai kebutuhan.</p>
                </div>
                ...
                <form action="{{ route('budgets.update', $budget) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input id="limit_amount" name="limit_amount" type="number" step="0.01" min="0" value="{{ old('limit_amount', $budget->limit_amount) }}" required class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100" placeholder="500000" />
                        ...
                        <div>
                            ...
                            <input id="month" name="month" type="number" min="1" max="12" value="{{ old('month', $budget->month) }}" required class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100" />
                        ...
                        <div>
                            ...
                            <input id="year" name="year" type="number" min="2000" value="{{ old('year', $budget->year) }}" required class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100" />
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:justify-end pt-4 border-t border-slate-200">
                        ...
                        <button type="submit" class="inline-flex justify-center rounded-2xl bg-sky-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-sky-700">Perbarui Budget</button>
                    </div>
                </form>
            </div>
        </div>
