...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Detail saldo, tipe akun, dan aktivitas terbaru.</p>
            </div>

            <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                <a href="{{ route('accounts.edit', $account) }}"
                    class="ui-button inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 shadow-sm hover:border-slate-300 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-300">
                ...
                <a href="{{ route('accounts.index') }}"
                    class="ui-button inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-slate-950 px-4 text-sm font-semibold text-white shadow-sm shadow-slate-900/10 hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M4 6h16" />
                        <path d="M4 12h16" />
    ...
    @php
        ...
        $typeStyles = [
            'bank' => ['line' => 'from-sky-500 to-cyan-400', 'icon' => 'bg-sky-50 text-sky-700 ring-sky-100', 'badge' => 'bg-sky-50 text-sky-700 ring-sky-100', 'amount' => 'text-sky-700'],
            'cash' => ['line' => 'from-emerald-500 to-teal-400', 'icon' => 'bg-emerald-50 text-emerald-700 ring-emerald-100', 'badge' => 'bg-emerald-50 text-emerald-700 ring-emerald-100', 'amount' => 'text-emerald-700'],
            'credit' => ['line' => 'from-rose-500 to-red-400', 'icon' => 'bg-rose-50 text-rose-700 ring-rose-100', 'badge' => 'bg-rose-50 text-rose-700 ring-rose-100', 'amount' => 'text-rose-700'],
            'other' => ['line' => 'from-amber-500 to-orange-400', 'icon' => 'bg-amber-50 text-amber-700 ring-amber-100', 'badge' => 'bg-amber-50 text-amber-700 ring-amber-100', 'amount' => 'text-amber-700'],
        ];
    ...
    <div class="min-h-screen bg-[#f6f7f9] py-6 text-slate-900 sm:py-10">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
                <section class="ui-reveal relative overflow-hidden rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                    ...
                    <div class="relative">
                        ...
                        <div class="mt-9 rounded-lg bg-slate-50 p-5 ring-1 ring-slate-200">
                            ...
                            <p class="mt-3 text-4xl font-bold tracking-tight {{ $balance < 0 ? 'text-red-600' : $style['amount'] }} sm:text-5xl">
                ...
                <section class="ui-reveal rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-6" style="animation-delay: .05s">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Transaksi Terbaru</p>
                    ...
                    <div class="mt-5 space-y-3">
                        @forelse ($recentTransactions as $transaction)
                            <div class="ui-card rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                                <div class="flex items-center justify-between gap-4">
                                    ...
                                    <p class="shrink-0 text-sm font-bold {{ $transaction->type === 'income' ? 'text-emerald-600' : 'text-red-600' }}">
                        ...
                        @empty
                            <div class="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-10 text-center">
                                <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-lg bg-white text-slate-400 shadow-sm ring-1 ring-slate-200">
            ...
            <section class="ui-reveal mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-6" style="animation-delay: .1s">
